# -*- coding: utf-8 -*-
"""
Created on Wed Oct 13 20:54:25 2021
...
def create_data_model():
    ...
    data = {}
    widths = [3,1,2,1,2,2,2]
    lengths = [3,1,1,3,2,2,1]
    heights=[1,1,2,2,2,2,1]
    ...
    data['bin_max_width'] = [2,3,3]
    data['bin_max_length'] = [2,3,3]
    data['bin_max_height']=[2,3,3]
    return data
# %%
def make_boxes(data,cube_origins,dimension_values_list,binj, bin_item_dict):
    ...
    for i in data['items']:
        try:
            if((cube_origins[binj]['x'][i]>=0) & (bin_item_dict['Item'+str(i)]['Indicator']==1)):
                cube=((x >= cube_origins[binj]['x'][i]) & (x <= cube_origins[binj]['x'][i]+dimension_values_list[0][i]-1) & 
                (y >= cube_origins[binj]['y'][i]) & (y <= cube_origins[binj]['y'][i]+dimension_values_list[1][i]-1) & 
                (z >= cube_origins[binj]['z'][i]) & (z <= cube_origins[binj]['z'][i]+dimension_values_list[2][i]-1))
                colors[cube]=clr[i]
                voxels |= cube
        except:
            continue
...
#                print('Packed bin value:', bin_value)
    ...
    print(buffer[idx])
    for j in data['bins']:
        make_boxes(data,buffer[idx]['origin'],orientation_dict[idx],j,buffer[idx]['Bin'+str(j)])
    return buffer[idx]
# %%
